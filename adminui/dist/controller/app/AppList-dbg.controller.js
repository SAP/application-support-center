sap.ui.define([
	...
	return BaseController.extend("asc.admin.controller.app.AppList", {
		...
		/*
		filterMyApps: function (oEvent) {
			var that = this;
			this.api.getMyApps(this.getOwnerComponent().getModel("user").getData().userId)
				.done(function (oData) {
					var oAppsModel = new sap.ui.model.json.JSONModel(oData);
					oAppsModel.setSizeLimit(1000);
					that.getOwnerComponent().setModel(oAppsModel, "apps");
		...
		*/
		...
		onClearFilters: function() {
			...
			this.getView().byId("idTableApps").getBinding("items").filter([]);
			this.byId("idFilterBar").setVisible(false);
			this.byId("idFilterLabel").setText("");
		},
		...
		filterList: function(sFilterString) {
			...
			if (sFilterString) {
				...
				//update list binding
				if (filters.length > 0) {
					this.getView().byId("idTableApps").getBinding("items").filter(new sap.ui.model.Filter(filters, true));
				} else {
					this.getView().byId("idTableApps").getBinding("items").filter([]);
				}
			}
